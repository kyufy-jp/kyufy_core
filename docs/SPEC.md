...
>
> Rev. 2.4 — amended after implementation review (retrieval recall, geographic scoping, income semantics, port contract, aggregation precedence, validity filtering, embedding testability, identifier exposure). Rev 2.1 adds: designated-city ward matching, step-0 normalization carve-out, document_ref pinned to integer index, filter_parameters tradeoff acknowledgment. Rev 2.2 adds: ancestor-residence third clause (coarse residence vs. finer program scope → 要確認, never drop), and a consistent Tokyo demo story (§7 example = 新宿区; さいたま市中央区 stays as the §9 ward fixture). Rev 2.3 adds: the carve-out verdict cap (programs admitted via carve-out are capped at 要確認 with a synthesized `residence_unverified` reason, even with no residence Requirement row). Rev 2.4 adds: residence requirements decided by the geographic admission (:match → 該当 with citation; ancestor/normalization-failure → 要確認; cap reason only when no residence row) — codifying PR #13, with the raw_text-nuance tradeoff acknowledged.
...
3.5 **Residence requirements are decided by the geographic admission, not RuleCheck**: a `kind: residence` requirement is a geographic condition the flat Profile cannot re-verify, so its verdict comes from step 0's admission — **:match → 該当 (citation kept)**; :ancestor / normalization-failure → 要確認 (consistent with the carve-out cap). Rationale: a full :match means the profile's residence is already confirmed in the program's scope; hiding a clearly-eligible resident behind 要確認 violates the prime directive (§0). **Acknowledged tradeoff**: residence raw_text may encode more than geography (住民登録, 各月1日時点 etc.) which is not re-verified — the always-displayed citation surfaces those nuances to the user, and the standing disclaimer covers the residual gap; this is a conscious 参考判定-consistent choice, not an oversight. The synthesized `residence_unverified` cap reason fires only when a carve-out program has **no** residence requirement of its own (no duplicates).
